...
class News_contenttypesapi_NewsArticlesPlugin extends contentTypeBase
{
    ...
    function display()
    {
        ...
        if (!empty($items)) {
            ...
            foreach (array_keys($items) as $k) {
                ...
                $items[$k]['readperm'] = (SecurityUtil::checkPermission('News::', "$items[$k][aid]::$items[$k][sid]", ACCESS_READ) || SecurityUtil::checkPermission('Stories::Story', "$items[$k][aid]::$items[$k][sid]", ACCESS_READ));
            }
    ...
    {
        $dom = ZLanguage::getModuleDomain('News');
        return array('title' => '', 'categories' => null, 'status' => 0, 'show' => 1, 'limit' => 5, 'order' => 0, 'dayslimit' => 0, 'maxtitlelength' => 0, 'titlewraptext' => '...', 'disphometext' => false, 'maxhometextlength' => 300,
            'hometextwraptext' => '[' . __('read more', $dom) . ']', 'dispuname' => false, 'dispdate' => true, 'dateformat' => '%x', 'dispreads' => false, 'dispcomments' => false, 'dispsplitchar' => ',', 'dispnewimage' => false, 'newimagelimit' => 3,
            'newimageset' => 'global', 'newimagesrc' => 'new_3day.gif', 'linktosubmit' => true);
    ...
    function startEditing(&$render)
    {
        ...
        $showoptions = array(array('value' => 1, 'text' => __('All', $dom)), array('value' => 2, 'text' => __('Lead page', $dom)), array('value' => 3, 'text' => __('Non lead page', $dom)));

        $statusoptions = array(array('value' => 0, 'text' => __('Published', $dom)), array('value' => 1, 'text' => __('Rejected', $dom)), array('value' => 2, 'text' => __('Pending Review', $dom)), array('value' => 3, 'text' => __('Archived', $dom)));

        $orderoptions = array(array('value' => 0, 'text' => __('News module setting', $dom)), array('value' => 1, 'text' => __('Number of reads', $dom)));
